...
<CFLOOP index="i" from="1" to="#ArrayLen(BenchData)#">
	<CFQUERY name="HDDB" datasource="mysql">
		SELECT ModelID
		FROM DiskSpeed.Models
		WHERE VendorID=(SELECT ID FROM DiskSpeed.Vendors WHERE Vendor='#BenchData[i].Vendor#' ORDER BY ID LIMIT 0,1)
	...
	<CFIF HDDB.RecordCount NEQ 0>
		<cflock name="BenchmarkInsert" type="exclusive" throwontimeout="true" timeout="60">
			...
			<CFIF StructKeyExists(BenchData[i],"SSDReadSpeedList") EQ "NO">
				<CFLOOP index="q" from="1" to="#ArrayLen(BenchData[i].BenchMark)#">
					<CFQUERY datasource="mysql">
						INSERT INTO DiskSpeed.Benchmarks
							(BenchmarkID, Spot, Speed)
						VALUES
							(#BenchID.ID#, #BenchData[i].BenchMark[q][1]#, #BenchData[i].BenchMark[q][2]#)
					</CFQUERY>
				</CFLOOP>
			<CFELSE>
				<CFIF StructKeyExists(BenchData[i],"BouncyDrive")>
					<CFQUERY datasource="mysql">
						INSERT INTO DiskSpeed.SSD_Benchmarks
							(BenchmarkID, TestFileSize, SSDReadSpeedList, SSDWriteSpeedList, SSDReadSpeed, SSDWriteSpeed, BouncyDrive, CacheDetected)
						VALUES
							(#BenchID.ID#, #Val(BenchData[i].TestFileSize)#, '#BenchData[i].SSDReadSpeedList#', '#BenchData[i].SSDWriteSpeedList#', '#BenchData[i].SSDReadSpeed#', '#BenchData[i].SSDWriteSpeed#',
							 #Val(BenchData[i].BouncyDrive)#, #Val(BenchData[i].CacheDetected)#)
					</CFQUERY>
					<CFSET EndIdx=ListLen(BenchData[i].SSDReadSpeedList)>
					<CFQUERY datasource="mysql">
						INSERT INTO DiskSpeed.SSD_ModelBenchmarks
							(ModelID, FileIdx, ReadMBs, WriteMBs)
						VALUES
						<CFLOOP index="q" from="1" to="#EndIdx#">
							(#HDDB.ModelID#, #q#, #Val(ListGetAt(BenchData[i].SSDReadSpeedList,q))#, #Val(ListGetAt(BenchData[i].SSDWriteSpeedList,q))#)
							<CFIF q LT EndIdx>,</CFIF>
						</CFLOOP>
					</CFQUERY>
				</CFIF>
			</CFIF>
